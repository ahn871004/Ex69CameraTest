package kr.co.ajsoft.ex69cameratest;
...
public class MainActivity extends AppCompatActivity {
    ...
    public void clickPhoto(View view) {
        ...
        //캡쳐 결과를 받을 수 있도록 Camera 액티비티를 실행하기
        ...
        switch (requestCode){
            case 100:
                ...
                if(resultCode==RESULT_OK){
                    ...
                    //결과물을 가져온 Intent객체(3번째 파라미터 : data에게 결과 받기)
                    ...
                    //파일로 저장하지 않고 Bitmap이미지 데이터로
                    //결과를 돌려주는 디바이스들이 많음

                    if(uri!=null){ //파일로 저장되는 방식일 때
                        ...
                        Toast.makeText(this, "uri로 사진정보 획득", Toast.LENGTH_SHORT).show();
                        Glide.with(this).load(uri).into(iv);

                    }else{//Bitmap 이미지 데이터만 줄때[요즘은 거의 이방식]

                        Toast.makeText(this, "bitmap으로 사진정보 획득", Toast.LENGTH_SHORT).show();

                        //Bitmap객체를 Intent로부터 Extra데이터로 전달 받음
